...
export function DemoStatsPage() {
  const stats = useMemo(() => {
    ...
    const byCategory: Record<string, { slug: CategorySlug; label: string; color: string; textColor: string; monthly: number; count: number }> = {}
    ...
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
              ...
              return (
                <div key={item.id} className="flex items-center gap-3">
                  ...
                  <p className="text-sm font-semibold tabular-nums">
                    {fmtMoney(monthly)}<span className="text-xs text-gray-400">/월</span>
                  </p>
                </div>
              )
            })}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-2">
        ...
        <p className="text-xs text-gray-400 leading-relaxed">
          카테고리별 월 지출 비율, TOP 5 지출 항목, 자동이체·AI파싱 통계를 자동으로 계산합니다.
          구독·보험·통신비·렌탈·세금·공과금 전체 생활비를 분석하여 절약 포인트를 찾아드립니다.
        </p>
      </div>
    </div>
  )
